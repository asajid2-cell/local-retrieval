...
MUX is local-first. It should treat source session files as read-only and store app metadata separately under `%LocalAppData%\CodexLocalRetrieval`. The existing `CodexLocalRetrieval` data path is an internal compatibility path and remains unchanged.
